    ...
    <div className="space-y-4 rounded-xl bg-white/10 p-4 backdrop-blur-sm">
            ...
            {Object.entries(demoNotifications).map(([type, content]) => (
              <SelectItem key={type} value={type as string} className="flex items-center">
                <div className="flex items-center gap-2">
                  <span className="inline-flex">{content.icon}</span>
      ...
      >
        <div className="flex items-center gap-2 font-medium">
          ...
          <h4 className={cn(!enabled && 'opacity-85')}>{currentNotif.title}</h4>
        </div>
        <p className={cn('mt-1 text-sm', !enabled && 'opacity-80')}>{currentNotif.body}</p>
        {!enabled && (
